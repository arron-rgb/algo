...
public class NumberOfNodesInTheSubTreeWithTheSameLabel {
  ...
  public static void main(String[] args) {
    Solution solution = new NumberOfNodesInTheSubTreeWithTheSameLabel().new Solution();
    String[] data = """
                  7
      ...
      "aabab"
                  """.trim().replaceAll("\n", "|").split("\\|");
    String[] paramTypes = InputUtil.param("[int, int[][], String]");
    ...
    for (int i = 0; i < loop; i++) {
      int[] q = solution.countSubTrees((int)params[1 - 1 + i * paramTypes.length],
        (int[][])params[2 - 1 + i * paramTypes.length], (String)params[3 - 1 + i * paramTypes.length]);
  ...
  class Solution {
    ...
    public int[] countSubTrees(int n, int[][] edges, String labels) {
      ...
      for (int[] edge : edges) {
        ...
        edgesMap.computeIfAbsent(node0, t -> new ArrayList<>()).add(node1);
        edgesMap.computeIfAbsent(node1, t -> new ArrayList<>()).add(node0);
      }
    ...
    public int[] dfs(int node, int[] counts, boolean[] visited, Map<Integer, List<Integer>> edgesMap, String labels) {
      ...
      List<Integer> nodesList = edgesMap.getOrDefault(node, List.of());
      // 把后面的都统计完
