...
public class MinimumNumberOfDaysToMakeMBouquets {
  ...
  // 1 <= k <= n
  //
  // Related Topics Array Binary Search 👍 1802 👎 42
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < loop; i++) {
      int q = solution.minDays((int[])params[1 + i * paramTypes.length - 1], (int)params[2 + i * paramTypes.length - 1],
  ...
  class Solution {
    ...
    private boolean check(int m, int k, int day, int[] bloomDay) {
      int flower = 0;
      // k: adjacent flowers
      int count = 0;
      for (int i : bloomDay) {
