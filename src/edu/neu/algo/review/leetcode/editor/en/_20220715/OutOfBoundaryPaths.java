...
public class OutOfBoundaryPaths {
  ...
  // adjacent cells in the grid (possibly out of the grid crossing the grid boundary). You
  ...
  // 0 <= startColumn < n
  //
  // Related Topics Dynamic Programming 👍 1634 👎 185
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < loop; i++) {
      int q = solution.findPaths((int)params[1 + i * paramTypes.length - 1], (int)params[2 + i * paramTypes.length - 1],
        (int)params[3 + i * paramTypes.length - 1], (int)params[4 + i * paramTypes.length - 1],
        (int)params[5 + i * paramTypes.length - 1]);
