...
public class CountCommonWordsWithOneOccurrence {
  ...
  //
  // Related Topics Array Hash Table String Counting 👍 315 👎 6
  ...
  class Solution {
    public int countWords(String[] words1, String[] words2) {
      Map<String, Long> map =
        Arrays.stream(words1).collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
      Map<String, Long> map2 =
        Arrays.stream(words2).collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
      return (int)Arrays.stream(words2).filter(s -> map.getOrDefault(s, 0L) == 1 && map2.getOrDefault(s, 0L) == 1)
        .count();
    }
  }
