...
public class CarFleet {
  ...
  // 0 < speed[i] <= 10⁶
  //
  // Related Topics Array Stack Sorting Monotonic Stack 👍 1414 👎 413

  public static void main(String[] args) {
    Solution solution = new CarFleet().new Solution();
    solution.carFleet(12, new int[] {10, 8, 0, 5, 3}, new int[] {2, 4, 1, 1, 3});
  }
  ...
  class Solution {
    public int carFleet(int target, int[] position, int[] speed) {
      ...
      for (int i = 0; i < n; ++i) {
        cars.add(new double[] {position[i], (double)(target - position[i]) / speed[i]});
      }
      // 按距离终点距离 desc 排序
      cars.sort(Comparator.comparing(i -> i[0]));

      // 从末尾访问
      int res = 1;
      for (int i = cars.size() - 1; i >= 0; i--) {
        if (cars.get(i)[1] < cars.get(i - 1)[1]) {
          res++;
        } else {
